...
% expand globs (only works under linux/unix)
sacfiles={};%k=1;
for i=1:length(fnames)%
  fnlist=cellstrnl(ls('-1',fnames{i}),char(10));
  sacfiles=[ sacfiles fnlist' ];
...
% normally external function cellstrnl - make available here to keep this
% code self-contained

function [c]=cellstrnl(str,varargin)
% CELLSTRNL convert newline-tokenised character row vector into cell array
%
% C = CELLSTRNL(string) where string is a character row vector
%   containing any number of newline characters will generate a
%   cell array where each newline separated line in string
%   will be one cell of C
%
% C = CELLSTRNL(string, token) does the same except that token is
%   used instead of newline.  token can be more than one character
%   long but note that the token is itself is not included in the
%   cells
...
ind=findstr(str,token);

if isempty(ind)
  ind=length(str)+1;
elseif ind(end)~=1+length(str)-length(token)
  ind(end+1)=1+length(str);
end

last=1;
for k=1:length(ind)
  c{k}=str(last:ind(k)-1);
  last=ind(k)+length(token);
end
