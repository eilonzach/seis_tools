...
name = 'pprem_disp'; % name of model
odir = '~/Documents/MATLAB/Classes/Adv_Seismo/';

ifanis = 1; % 1 if there is some (radial) anisotropy, 0 otherwise
tref = 1.;  % reference period (recommend 1) for physical dispersion

N = 749; % nominal no. of nodes - may change due to rounding
...
fid = fopen(ofile,'w+');
fprintf(fid,'%s\n',name);
fprintf(fid,'%u %.4f 1 1\n',ifanis,tref);
fprintf(fid,'%u %u %u %u %u\n',N, sum(Nk(1:1)), sum(Nk(1:2)), sum(Nk(1:9)), sum(Nk(1:11)));
for kk = 1:N
    fprintf(fid,'%7.0f. %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.5f \n',...
