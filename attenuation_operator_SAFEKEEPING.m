...
elseif alpha>0
    cta = cot(0.5*pi*alpha);
    qinv_w = qinv.*abs(w).^(-alpha);
    ...
    c_w = c0 * (1 - 0.5*cta*qinv_w*(2*pi)^alpha); 
%     c_w = c0 * (1 - 0.5*cta*qinv*abs(w/2/pi).^-alpha); 
    c_inf = c0; % precise, because the f-dependent term is causal

    % USING Lekic 2009 eq(11) with reference freq w=1;
%     c_w = c0 * (1 + (0.5*cta*qinv)*(1 - abs(w/2/pi).^-alpha) );
% 	  c_inf = c0 * (1 + (0.5*cta*qinv) );
end
