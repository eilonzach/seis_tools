function [J1,J2,tau_ss] = creep10_ZE_wsquirt(Te,gs,pres,omega,vfac,ipk)
%function [J1,J2] = creep10_ZE_wsquirt(Te,gs,pres,omega,vfac, ipk)
...
global alpha sig sigsq; % alpha is used in J1anel and J2anel, sig in J1p and J2p
...
% BOOSTED peak parameters:
tauPo = 3.98E-4; % reference peak relaxation time,
deltaP = 1 * 0.057;  % %0.057 from JF10 peak relaxation strength pref
sig    = 1 * 4;         % %4 %peak width
EB = 4.5E5; % activation energy for background and peak

% DISABLED squirt peak parameters:
tauSQ = 0.1; % reference peak relaxation time,
deltaSQ = 0.000;  % peak relaxation strength pref
sigsq = 0.01;         % peak width
...
csq = deltaSQ * (2*pi)^(-0.5)/sigsq; %peak integration const.
...
% ZE - added squirt peak contribution to absorption spectrum
for ii = 1 : length(Te)
  ...
  isq1(ii) = quadgk(@(tau)J1sqirt(tau,omega(ii),tauSQ),0,inf);
  isq2(ii) = quadgk(@(tau)J2sqirt(tau,omega(ii),tauSQ),0,inf);
end
