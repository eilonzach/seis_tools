...
ij1_cell = arrayfun(@(omega_val, tauL_val, tauH_val) integral(@(tau) J1anel(tau, omega_val), tauL_val, tauH_val), omega_v, tauL_v, tauH_v, 'UniformOutput', false);
ij1_v = cell2mat(ij1_cell);
ij2_cell = arrayfun(@(omega_val, tauL_val, tauH_val) integral(@(tau) J2anel(tau, omega_val), tauL_val, tauH_val), omega_v, tauL_v, tauH_v, 'UniformOutput', false);
ij2_v = cell2mat(ij2_cell);
ip1_cell = arrayfun(@(omega_val, tauP_val) quadgk(@(tau) J1p(tau, omega_val, tauP_val), 0, inf), omega_v, tauP_v, 'UniformOutput', false);
ip1_v = cell2mat(ip1_cell);
ip2_cell = arrayfun(@(omega_val, tauP_val) quadgk(@(tau) J2p(tau, omega_val, tauP_val), 0, inf), omega_v, tauP_v, 'UniformOutput', false);
ip2_v = cell2mat(ip2_cell);

ij1 = reshape(ij1_v,size(T));
ij2 = reshape(ij2_v,size(T));
ip1 = reshape(ip1_v,size(T));
ip2 = reshape(ip2_v,size(T));
