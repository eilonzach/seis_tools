...
for ip = 1:length(phases)
    ...
    reqfile{ip} = [label{ip},'_request'];
    if ifrequest
        breq_fast_request(label{ip},'zeilon',station,chans,network,[],time0(:,ip),time1(:,ip),'SEED',reqfile{ip})
        fprintf('%s request sent\n',label{ip});
    end
end% loop on phases
