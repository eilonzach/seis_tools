function [solution] = solve_for_stable_melt_fraction_ze(T_K,P_GPa,C_h2o_solid,C_co2_solid,D_h2o,D_co2,solidus_str)
...
%
% jbrussell 2023
%  arguments block added by Zach Eilon, 2023
%  I think solution.phi is in weight fraction.
arguments
    T_K (:,1) {mustBeNumeric}
    P_GPa (:,1) {mustBeNumeric,mustBeLessThan(P_GPa,100)}
    C_h2o_solid (:,1) {mustBeNumeric}
    C_co2_solid (:,1) {mustBeNumeric}
    D_h2o (1,1) {mustBeNumeric} = 0.01
    D_co2 (1,1) {mustBeNumeric} = 1e-6
    solidus_str {mustBeText} ='katz'
end
